...
if( ! class_exists( 'AMT_POST_VIEWS' )){
	...
	class AMT_POST_VIEWS{
		...
		function _set_post_views(){
			...
			if( !MedixareTheme::$options['post_view'] || ! is_single() ){
				return;
			}
			...
			/*if( !MedixareTheme::$options['post_view'] || $page > 1  ){
				return false;
			}*/
		...
		function _posts_column_views( $defaults ){
			...
			if( MedixareTheme::$options['post_view'] ){
				$defaults['post_view'] = esc_html__( 'Views', 'medixare-core' );
			}
		...
		function _posts_custom_column_views( $column_name, $id ){
			if( $column_name === 'post_view' ){
				echo medixare_views( '', get_the_ID() );
			}
		}
	}
...
/*----------------------------------------------------------*/
if( !function_exists( 'medixare_views' )){

	function medixare_views( $text = '', $post_id = 0 ){
		...
		if ( $view_count == 1 ) {
			$medixare_view_html = esc_html__( 'View' , 'medixare-core' );
		} else {
			$medixare_view_html = esc_html__( 'Views' , 'medixare-core' );
		}
		
		$medixare_views_html = '<span class="view-number" >' . $view_count . '</span> ' . $medixare_view_html;

		return '<span class="meta-views meta-item '. $views_class .'">'. $medixare_views_html.'</span> ';
	}
}
