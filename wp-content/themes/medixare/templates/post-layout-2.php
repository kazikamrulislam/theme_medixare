				...
				<div class="entry-thumbnail-area <?php echo esc_attr($img_class); ?>">
					<?php if ( MedixareTheme::$options['post_featured_image'] == true ) {
						if ( has_post_thumbnail() ) {
							the_post_thumbnail( 'medixare-size1' ); 
						} 
					 } ?>
					...
					<div class="entry-header">						
						<?php if ( MedixareTheme::$options['post_cats'] ) { ?><span class="entry-categories style-1"><?php echo medixare_single_category_prepare(); ?></span><?php } ?>
						<h1 class="entry-title title-size-lg title-light-color"><?php the_title();?></h1>
				...
				<?php if ( MedixareTheme::$layout == 'left-sidebar' ) { get_sidebar(); } ?>
					<div class="<?php echo esc_attr( $medixare_layout_class );?> <?php echo esc_attr( $sidebar_order ) ?>">
						<main id="main" class="site-main">
							<div class="rt-sidebar-space post-detail-style3 <?php echo ( MedixareTheme::$options['scroll_post_enable'] == 1 ) ? esc_attr( 'ajax-scroll-post' ) : ''; ?>">
