...
* By : Radius Theme
*/
Class MedixareTheme_Feature_Post extends WP_Widget {
	public function __construct() {
		$widget_ops = array(
			'classname' => 'rt-feature-post',
			'description' => esc_html__( 'Feature Post Display Widget' , 'medixare-core' ),
			'customize_selective_refresh' => true,
		);
		parent::__construct( 'rt-feature-post', esc_html__( 'Medixare : Feature Posts display' , 'medixare-core' ), $widget_ops );
	}
	public function widget( $args, $instance ) {
		...
		$title = ( !empty( $instance['title'] ) ) ? $instance['title'] : esc_html__( 'Recent Posts' , 'medixare-core' );
		...
		<?php while ( $result_query->have_posts() ) {
			...
			<div class="rt-feature-widget">
					...
					<?php if ( $show_post_image == 'yes' ) { ?>
						<a class="post-img-holder img-opacity-hover" href="<?php the_permalink(); ?>" title="<?php the_title_attribute(); ?>"><?php
								if ( has_post_thumbnail() ){
									the_post_thumbnail( 'medixare-size4', ['class' => 'media-object'] );
						...
						if ( $show_cat ) { ?>
							<span class="entry-categories style-1 position-absolute">
								...
								foreach ( $term_lists as $term_list ){
									...
									<?php if ( $i > 1 ){ echo esc_html( ', ' ); } ?><a href="<?php echo esc_url( $link ); ?>"><span class="category-style"><?php echo esc_html( $term_list->name ); ?></span></a>
							<?php $i++; } ?></span>
							<?php } ?>
					<?php } ?>
				</div>
				<div class="media-body entry-content">
					...
					<ul class="entry-meta">
						<?php if ( $show_date ) { ?>
						<li class="post-date"><i class="far fa-calendar-alt"></i><?php echo get_the_time( get_option( 'date_format' ) ); ?></li>
						<?php } ?>
					</ul>
					<?php } ?>
