	...
	if( get_post_type() != 'page' ) { 
		...
		$share_buttons = array(
			...
			'facebook-f' => array(
				...
				'text' => esc_html__( 'Facebook', 'medixare-core' ),
			),

			'twitter' => array(
				'url'   => 'https://twitter.com/intent/tweet?text='. $post_title .'&amp;url='. $post_link, 
				'text'  => esc_html__( 'Twitter', 'medixare-core' ),
			),

			'youtube' => array(
				'url'   => 'https://www.youtube.com?text='. $post_title .'&amp;url='. $post_link, 
				'text'  => esc_html__( 'Youtube', 'medixare-core' ),
			),

			'linkedin-in' => array(
				'url'   => 'http://www.linkedin.com/shareArticle?mini=true&amp;url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'LinkedIn', 'medixare-core' ),
			),
			
			'pinterest' => array(
				'url'   => 'http://pinterest.com/pin/create/button/?url='. $post_link .'&amp;description='. $post_title .'&amp;media='. medixare_post_img_src( 'medixare-size1' ),
				'text'  => esc_html__( 'Pinterest', 'medixare-core' ),
			),
			
			'whatsapp' => array(
				'url'   => 'https://api.whatsapp.com/send?text='. $post_title . ' – ' . $post_link ,
				'text'  => esc_html__( 'Whatsapp', 'medixare-core' ),
			),

			'cloud' => array(
				'url'   => 'https://cloud.google.com/send?text='. $post_title . ' – ' . $post_link ,
				'text'  => esc_html__( 'Cloud', 'medixare-core' ),
				'icon'  => 'fas fa-cloud',
			),

			'dribbble' => array(
				'url'   => 'https://dribbble.com?url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'StumbleUpon', 'medixare-core' ),
			),

			'tumblr' => array(
				'url'   => 'http://www.tumblr.com/share/link?url='. $post_link .'&amp;name='. $post_title,
				'text'  => esc_html__( 'Tumblr', 'medixare-core' ),
			),

			'reddit' => array(
				'url'   => 'http://reddit.com/submit?url='. $post_link .'&amp;title='. $post_title,
				'text'  => esc_html__( 'Reddit', 'medixare-core' ),
			),
		);
		
		if ( MedixareTheme::$options['post_share_facebook'] == 0 ){
		unset($share_buttons['facebook-f']);
		}
		if ( MedixareTheme::$options['post_share_twitter'] == 0 ){
		unset($share_buttons['twitter']);
		}
		if ( MedixareTheme::$options['post_share_youtube'] == 0 ){
		unset($share_buttons['youtube']);
		}
		if ( MedixareTheme::$options['post_share_linkedin'] == 0 ){
		unset($share_buttons['linkedin-in']);
		}
		if ( MedixareTheme::$options['post_share_pinterest'] == 0 ){
		unset($share_buttons['pinterest']);
		}
		if ( MedixareTheme::$options['post_share_whatsapp'] == 0 ){
		unset($share_buttons['whatsapp']);
		}
		if ( MedixareTheme::$options['post_share_cloud'] == 0 ){
		unset($share_buttons['cloud']);
		}
		if ( MedixareTheme::$options['post_share_dribbble'] == 0){
		unset($share_buttons['dribbble']);
		}
		if ( MedixareTheme::$options['post_share_tumblr'] == 0 ){
		unset($share_buttons['tumblr']);
		}
		if ( MedixareTheme::$options['post_share_reddit'] == 0 ){
