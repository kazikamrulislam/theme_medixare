...
$thumb_size1 = 'medixare-size5';
$thumb_size2 = 'medixare-size4';
	...
	<?php $count = 1; $i = $data['delay']; $j = $data['duration']; 
		...
		<?php while ( $query->have_posts() ) : $query->the_post();?>
			...
			<?php if ( $count == 1 ) { ?>
				<div class="col-xl-5 col-lg-6">
					<div class="amt-item-wrap <?php echo esc_attr( $data['animation'] );?> <?php echo esc_attr( $data['animation_effect'] );?>" data-wow-delay="<?php echo esc_attr( $i );?>s" data-wow-duration="<?php echo esc_attr( $j );?>s">
						<div class="amt-item amt-item-single">
							<div class="amt-image">
								...
								<?php if ( $data['post_category'] == 'yes' ) { ?>
									...
									<span class="entry-categories style-2 position-absolute meta-light-color"><?php echo the_category( ', ' );?></span>
									<?php } ?>
								<?php } ?>											
							</div>
			...
			<?php } ?>
				...
				<?php if ( $count > 1 ) { ?>
					<div class="col-xl-6 col-lg-12 col-md-6 col-12 <?php echo esc_attr( $data['animation'] );?> <?php echo esc_attr( $data['animation_effect'] );?>" data-wow-delay="<?php echo esc_attr( $i );?>s" data-wow-duration="<?php echo esc_attr( $j );?>s">
						<div class="amt-item amt-item-list">
							<div class="amt-image">
								...
								<?php if ( $data['small_post_category'] == 'yes' ) { ?>
									...
									<span class="entry-categories style-1 position-absolute"><?php echo medixare_category_prepare(); ?></span>
									<?php } elseif ( $data['cat_layout'] == 'cat_layout2' ) { ?>
									<span class="entry-categories style-2 position-absolute meta-dark-color"><?php echo the_category( ', ' );?></span>
									<?php } ?>
								<?php } ?>									
							</div>
							<div class="entry-content">
								...
								<?php } ?>
								<h3 class="entry-title title-dark-color title-size-md"><a href="<?php the_permalink();?>"><?php echo esc_html( $small_title );?></a></h3>
							</div>
						</div>	
					</div>
				<?php } ?>
