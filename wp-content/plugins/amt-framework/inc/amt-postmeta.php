...
require_once 'amt-postmeta-fields.php';

if( !class_exists( 'AMT_Postmeta' ) ){

	class AMT_Postmeta {
		protected static $instance = null;
		public  $version          = AMT_FRAMEWORK_VERSION;
		...
		private $nonce_action     = 'amt_metabox_nonce';
		private $nonce_field      = 'amt_metabox_nonce_secret';

		private function __construct() {
			$this->fields_obj = new AMT_Postmeta_Fields();
		...
		public function add_meta_box( $id, $title, $post_types, $callback = '', $context = '', $priority = '', $fields = '' ) {
			...
			$fields = apply_filters( 'amt_postmeta_field_' . $id, $fields );
			$metaboxes = array(
				'title'         => $title,
				'callback'      => $callback,
			...
			$this->metaboxes[$id] = apply_filters( 'amt_metabox_' . $id, $metaboxes );
