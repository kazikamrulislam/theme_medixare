	...
	<?php $i = $data['delay']; $j = $data['duration']; 
		...
		<?php while ( $query->have_posts() ) : $query->the_post();?>
			...
			<div class="<?php echo esc_attr( $data['animation'] );?> <?php echo esc_attr( $data['animation_effect'] );?>" data-wow-delay="<?php echo esc_attr( $i );?>s" data-wow-duration="<?php echo esc_attr( $j );?>s">
					<div class="amt-item single-post-item <?php echo esc_attr( $data['list_border'] );?>">
						<div class="entry-content">		
							...
							<h3 class="entry-title title-dark-color title-size-md mb-0"><a href="<?php the_permalink();?>"><?php echo esc_html( $title );?></a></h3>
							<a href="<?php the_permalink();?>" class="icon-box"><i class="fas fa-chevron-right"></i></a>
							</div>						
						</div>
					</div>
				</div>
