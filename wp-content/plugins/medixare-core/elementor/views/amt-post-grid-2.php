...
global $post;
$medixare_has_entry_meta  = ($data['post_category'] == 'yes' || $data['post_author'] == 'yes' && function_exists('medixare_views')) ? true : false;
...
<div class="amt-post-grid-2 amt-post-grid-<?php echo esc_attr($data['style']); ?>">
	<div class="row <?php echo esc_attr($data['item_gutter']); ?>">
		...
		if ($query->have_posts()) : ?>
			<?php while ($query->have_posts()) : $query->the_post(); ?>
				...
				?>
				<div class="amt-post-grid__items col-xl-4 col-md-6 col-sm-6 col-xs-12 <?php //echo esc_attr($col_class); ?> <?php echo esc_attr($data['animation']); ?> <?php echo esc_attr($data['animation_effect']); ?>" data-wow-delay="<?php echo esc_attr($i); ?>s" data-wow-duration="<?php echo esc_attr($j); ?>s">
					<div class="amt-post-grid__item">
						<div class="amt-post-grid__thumb w-img">
						...
						<div class="amt-post-grid__content">
							<?php if ($data['post_read'] == 'yes') { ?>
								<div class="amt-post-grid__btn-wrapper"><a class="tp-readmore-btn" href="<?php the_permalink(); ?>"><?php echo wp_kses_post($data['read_more_text']); ?></a></div>
							<?php } ?>
							...
							<?php if ($medixare_has_entry_meta) { ?>
								<ul class="entry-meta">
									...
									<?php } if ($data['post_author'] == 'yes') { ?>
										<li class="post-author"><div class="author_image"><?php echo get_avatar( $author, 50 ); ?></div> <?php the_author_posts_link(); ?></li>
									<?php } ?>
								</ul>
							<?php } ?>
						</div>
